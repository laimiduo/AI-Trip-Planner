...
4. 餐饮 name 写具体菜品或店名（如"老北京炸酱面"），description 不超过20字

**JSON 结构:**
- city, start_date, end_date
- days[]: 每个 day 包含 date, description(10字内), attractions[], meals[], hotel, transportation(字符串)
- transportation_cost: 整数（当天交通费用）
- meals[]: {"name", "type"(breakfast/lunch/dinner), "description"(20字内), "estimated_cost"}
- hotel: {"name", "address", "estimated_cost"(一晚价格)}
- attractions[]: {"name", "address", "description"(20字内), "visit_duration"(分钟), "ticket_price"}
